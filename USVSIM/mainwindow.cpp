...
void MainWindow::socket_Read_Data()
{
    ...
    for (int i = 0; i < buffer.length(); i++)
    {
        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, static_cast<unsigned char>(c), &msg, &status))
        {
            ...
            break;
            case MAVLINK_MSG_ID_GCS_SIMULATION_STATUS: // 153: 仿真进行时
            {
            ...
            break;
            case MAVLINK_MSG_ID_GCS_SET_OR_USV_ACK: // 仿真初始化
            {
...
void MainWindow::simulateStep()
{
    ...
    if (usv.done)
        timer->stop();
    qDebug() << "done" << usv.done << "stage:" << usv.stage << "desiredX:" << usv.desiredX << "desiredY:" << usv.desiredY << usv.x << usv.y << usv.lng << usv.lat << usv.desired_course << usv.cmdTurn;
    mavlink_message_t msg;
    unsigned char buf[25];
